...
let DuctilityService = DuctilityService_1 = class DuctilityService {
    ...
    verifyInitDuctility(body) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const result = yield this.generalData_Service.verifyInitDuctility(body);
                return { result };
            }
            catch (error) {
    ...
    calculateDuctility(body) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                return yield this.calc_Service.calculateDuctility(body);
            }
            catch (error) {
                const { status, name, message } = error;
    ...
    saveEssay(body) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const alreadyExists = yield this.Ductility_Repository.findOne({
                    'generalData.name': name,
                    'generalData.material._id': materialId,
                    'generalData.userId': userId,
                });
                if (alreadyExists)
                    throw new exceptions_1.AlreadyExists(`DUCTILITY with name "${name}" from user "${userId}"`);
                const ductility = yield this.Ductility_Repository.create(body);
