...
export class Calc_Rtfo_Service {
  ...
  async calculateRtfo({ rtfo, generalData }: Calc_Rtfo_Dto): Promise<{ success: boolean; result: Calc_Rtfo_Out }> {
    try {
      ...
      if (
        (material.description.classification_CAP === 'CAP 30/45' ||
        ...
        alerts.push(
          `A variação de massa para o ${material.description.classification_CAP} não deve ser superior a 0,5%.`,
        );
      }
      if (
        (material.description.classification_AMP === 'AMP 50/65' ||
        ...
        alerts.push(
          `A variação de massa para o ${material.description.classification_AMP} não deve ser superior a 1%.`,
        );
      }
