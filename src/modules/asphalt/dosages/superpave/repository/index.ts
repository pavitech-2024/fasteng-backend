...
export class SuperpaveRepository {
  constructor(
    @InjectModel(Superpave.name, DATABASE_CONNECTION.ASPHALT) private superpaveModel: Model<SuperpaveDocument>,
  ) {}
  ...
  async saveStep(superpave: any, step: number): Promise<void> {
    try {
      await this.superpaveModel.updateOne({ _id: superpave._id }, { $set: { 'generalData.step': step } });
    } catch (error) {
