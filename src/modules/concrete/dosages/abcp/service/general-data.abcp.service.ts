...
export class GeneralData_ABCP_Service {
  ...
  async verifyInitABCP(abcp: ABCPInitDto) {
    const { name, userId } = abcp;
    try {
      ...
      const createdPartialAbcp = await this.abcpRepository.createPartialAbcp(name, userId);

      await this.abcpRepository.saveStep(createdPartialAbcp, 1);
