    ...
    async getEssays({ cement, coarseAggregate, fineAggregate }: ABCPEssaySelectionDto) {
        try {
            ...
            const cementData = materials.find((material) => {
                const { name } = material;
                if (material._id.toString() === cement.toString()) {
                    return {
                        cement,
                        name,
                    }
            ...
            const coarseGranulometrys = await this.granulometry_repository.findAllGranulometrysByMaterialId( coarseAggregate.id, 'coarse' );
