...
export class Calc_CONCRETERT_Service {
    ...
    constructor(private readonly rtRepository: ConcreteRtRepository, private readonly materialsRepository: MaterialsRepository) { }

    async calculateRt({ step2Data }: Calc_CONCRETERT_Dto): Promise<{ success: boolean; result: Calc_CONCRETERT_Out }> {
        try {
            this.logger.log('calculate rt on calc.rt.service.ts > [body]');
