...
export class StabilizedLayersSamplesService {
  ...
  async createSample(sample: any): Promise<StabilizedLayers_Sample> {
    try {
      // verifica se existe uma sample com mesmo nome e userId no banco de dados
      if (await this.stabilizedLayers_SamplesRepository.findOne({ name: sample.generalData.name }))
        throw new AlreadyExists(`Stabilized layer sample with name "${sample.generalData.name}"`);
